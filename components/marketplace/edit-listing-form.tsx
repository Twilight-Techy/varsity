...
import { updateListing } from "@/lib/actions/marketplace-actions"
...
export default function EditListingForm({ item }: EditListingFormProps) {
  const router = useRouter()
  const { toast } = useToast()
  const [isSubmitting, setIsSubmitting] = useState(false)
  const [images, setImages] = useState<string[]>(item.images || ["/placeholder.svg?height=300&width=300"])
  ...
  const onSubmit = async (values: FormValues) => {
    ...
    try {
      ...
      // Update the listing
      const result = await updateListing(item.id, itemData)
