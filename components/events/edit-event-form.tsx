...
import { updateExistingEvent } from "@/lib/actions/event-actions"
...
export default function EditEventForm({ event }: EditEventFormProps) {
  ...
  const parseEventDate = () => {
    try {
      return event.date ? parse(event.date, "MMMM d, yyyy", new Date()) : undefined
    } catch (error) {
  ...
  const parseEventTime = () => {
    if (!event.time) return { startTime: "", endTime: "" }
  ...
  const { startTime, endTime } = parseEventTime()
  ...
  const form = useForm({
    defaultValues: {
      ...
      type: event.isVirtual ? "virtual" : "in-person",
      location: event.isVirtual ? "" : event.location,
      isVirtual: event.isVirtual,
      virtualLink: event.isVirtual ? event.location : "",
      date: eventDate,
      startTime,
      endTime,
  ...
  const onSubmit = async (data: any) => {
    ...
    try {
      ...
      // Update the event
      const result = await updateExistingEvent(event.id, eventData)
      ...
      if (result.success) {
        ...
        toast({
          title: "Error",
          description: result.message || "Failed to update event. Please try again.",
          variant: "destructive",
        })
      }
    } catch (error) {
              ...
              <div className="border-2 border-dashed rounded-lg p-6 text-center">
                {event.image && (
                  <div className="mb-4">
                    ...
                    <div className="relative h-40 w-full max-w-md mx-auto rounded-md overflow-hidden">
                      <img
